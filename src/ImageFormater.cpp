...
void ImageFormater::applyThicken(int8_t* img, uint width, uint height) {
	//make adjaent pixels also white
	 int8_t * copy= new int8_t [160*120];
...
void ImageFormater::crop20by20Digit(int8_t* img, uint width, uint height, int8_t* outImg) {
	...
	uint centerOfMassX = GetCenterOfMassX(img, 160, 120, THRESHOLD);
	uint centerOfMassY = GetCenterOfMassY(img, 160, 120, THRESHOLD);
...
#endif
	int radius = GetBoundingBoxRadius(img, 160, 120, centerOfMassX, centerOfMassY);
